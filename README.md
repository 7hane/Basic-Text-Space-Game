...
# Based on a board game called Space Crusade I used to play 25+ years ago...
...
# Dice? Random
# Could the squad members have different attributes? Sniper, medic etc
# Use list/tuple to record attributes for each member or maybe numbers for a combination/clue to be used.....
# in safe, finds document which has to be opened, read and copied to a file and then 
# read and utilised! Dont forget to close files!! Find out if it exists!! Scan...!!
# write a function that returns a value, maybe decrypts a cypher!! and then gets passed to
# to another funct... Could make part of story!!!???
# use seek, deque, append, use dictionary and NamedTuple!!!??? Maybe to collect codes
# use enumerate, xrange, try, except VauleError etc, split() machine logic, FOR LOOP
